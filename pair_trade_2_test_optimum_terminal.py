...
for i in range(ma_len[0]):
	my_list = price[(n_fit - ma_len[0] + i):(n_fit+ i)]
	ma_series[i] = ma(my_list)
print(ma_series[0:(ma_len[0]+1)])

# Compute the first ma_len moving average
for i in range(std_dev_len[6]):
	my_list = price[(n_fit - std_dev_len[6] + i):(n_fit+ i)]
	std_dev_series[i] = std_dev(my_list)
print(std_dev_series[0:(std_dev_len[6]+1)])
...
for i in range(len(price_t)):   
	...
	if abs(ma_series[i]) <= 1e-9:
		ma_series[i] = ma(price_t[(i-ma_len[0]):i])

	# Calculate the sample standard deviation
	if abs(std_dev_series[i]) <= 1e-9:
		#print "The price series used to compute standard deviation is ", price_t[(i-std_dev_len[6]):i]
		std_dev_series[i] = std_dev(price_t[(i-std_dev_len[6]):i])
	...
	if position == 0:
		...
		if price_t[i] <= lower(ma_series[i], std_dev_series[i], std_dev_coeff[0]):
			print(tradeDate_t[i])
...
#              abs(sec_2_position)*row["closePrice_y"]*unit*comm_rate
		...
		elif price_t[i] >= upper(ma_series[i], std_dev_series[i], std_dev_coeff[0]):
			print(tradeDate_t[i])
			print("Portfolio price is: ", price_t[i])
			print("Moving average of the past data is", ma_series[i])
			# short the portfolio
...
#              abs(sec_2_position)*row["closePrice_y"]*unit*comm_rate
	...
	# (in the case of holding position, book_series will not be empty)
		...
		if position > 0 and ( price_t[i] >= upper(ma_series[i], std_dev_series[i], std_dev_coeff[0]) ):
...
#             print "Cumulative commission is ", commission
		...
		elif position < 0 and ( price_t[i] <= lower(ma_series[i], std_dev_series[i], std_dev_coeff[0]) ):
...
#              abs(sec_2_position)*row["closePrice_y"]*unit*comm_rate         
		...
		elif position > 0 and  ( price_t[i] <= stop_at_low(state, std_dev_series[i], sl_coeff[9]) ):
		...
		elif position < 0 and  ( price_t[i] >= stop_at_high(state, std_dev_series[i], sl_coeff[9]) ):
...
k1_upper = np.add(ma_series, std_dev_series*std_dev_coeff[0])
k1_lower = np.subtract(ma_series, std_dev_series*std_dev_coeff[0])
